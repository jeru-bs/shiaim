...
const S = {
  ...
  token:     null,   // session token (kept in sessionStorage)
  projects:  [],
  statuses:  [],
  changes:   [],
  lastSeen:  null,
...
async function apiCall(action, data = {}) {
  ...
  if (action !== 'login' && S.token) payload.token = S.token;
...
async function login(username, password) {
  ...
  try {
    const res = await apiCall('login', { user: key, pass: password });
    if (!res.ok || !res.token) return false;
    S.token = res.token;
    S.user  = res.user || { username: key };
...
async function submitChangePassword() {
  ...
  try {
    await apiCall('changePassword', { currentPassword: current, newPassword: newPw });
    closeModalEl(document.getElementById('change-password-modal'));
    toast('הסיסמה עודכנה. יש להתחבר מחדש.', 'success');
    forceLogout();
  } catch (e) {
    if (e.message === 'wrong_current_password') errEl.textContent = 'הסיסמה הנוכחית שגויה';
    else if (e.message === 'weak_password')      errEl.textContent = 'הסיסמה חייבת להכיל לפחות 6 תווים';
    else if (e.message === 'unauthorized')       errEl.textContent = 'הסשן פג תוקף — יש להתחבר מחדש';
    else                                         errEl.textContent = 'שגיאה בעדכון הסיסמה';
  }
}

async function logout() {
  try { if (S.token) await apiCall('logout', { token: S.token }); } catch (e) {}
  forceLogout();
}
...
function forceLogout(reason) {
  S.user = null;
  S.token = null;
  sessionStorage.removeItem('shiaim_token');
  sessionStorage.removeItem('shiaim_user');
  S.projects = [];
  S.changes  = [];
  S.statuses = [];
...
async function restoreSession() {
  const token    = sessionStorage.getItem('shiaim_token');
  const userJson = sessionStorage.getItem('shiaim_user');
  ...
  try {
    ...
    // apiCall already cleared the session on 'unauthorized'
  }
  S.token = null;
  return false;
}
...
function wireEvents() {
  ...
  document.getElementById('login-form').addEventListener('submit', async e => {
    ...
    try {
      ...
      errEl.textContent = 'שגיאת התחברות';
    }
  });
...
async function init() {
  ...
  try { localStorage.removeItem('shiaim_user'); } catch (e) {}
